...
export default function Video({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const reply = await axios.post("http://localhost:5300/get_token", {
        ...
        try {
          ...
          let layouts = joinDetails.mod ? (await roomSession.getLayouts()).layouts : [];
        ...
        setIsLoading(false);
        console.log(e);
        alert("Error encountered. Are you sure the backend is started?");
      }
    })();
